...
type UploadResult struct {
	...
	TaskId        string                 `protobuf:"bytes,13,opt,name=task_id,json=taskId,proto3" json:"task_id,omitempty"` // Persistent upload task, echoed by Acknowledgement.
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}
...
func (x *UploadResult) GetTaskId() string {
	if x != nil {
		return x.TaskId
	}
	return ""
...
//
// Size: a rule snapshot scales with rule count (tens of rules ≈ KBs). Far
// beyond gRPC's default 4MB receive limit the single message fails WHOLE —
// intentionally a loud failure (disconnect, reconnect, resync), never a
// partial delivery. Notably better than per-message pushes, whose failure
// mode above the buffer was silent loss.
...
const file_proto_v1_agent_proto_rawDesc = "" +
	...
	"\afs_type\x18\x04 \x01(\tR\x06fsType\"\xbd\x03\n" +
	...
	"retryCount\x12\x17\n" +
	"\atask_id\x18\r \x01(\tR\x06taskId\"\x8c\x01\n" +
