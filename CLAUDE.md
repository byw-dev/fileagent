...
跨平台前提：**不要假设 `/tmp` 的存在、位置、是否为内存盘、或是否会被自动清理。**
各平台差异极大（tmpfs 或磁盘；重启清空、按天清理、或永不清理；真目录或符号链接），
任何一条都不能当成默认值——历史上因此在开发机上攒出过数 GB 永不回收的构建缓存与仓库副本。

- **临时文件放在工作树内已被 `.gitignore` 忽略的目录**（如 `bin/`）。
  这样 `git status` 保持干净，且**删除工作树即完成清理**，不需要额外的清理步骤。
- **需要临时目录的工具：设 `TMPDIR` 指向该目录**，不要逐个工具去配。
  `mktemp` 与绝大多数 Unix 工具、Go 工具链都尊重 `$TMPDIR`。
- **禁止覆盖 `GOCACHE`**，使用工具链默认值（共享且常热，跨分支复用还能加速编译）。
  为每个分支/每轮实验各建一个缓存目录是数 GB 级别的浪费。
- **需要整份仓库副本时用 `git worktree add`**，不要 `cp -R` 到临时目录。
- **确实必须写到工作树之外时**，两者缺一不可：
  ```bash
  d=$(mktemp -d "${TMPDIR:-/tmp}/fileagent.XXXXXX")
  trap 'rm -rf "$d"' EXIT INT TERM
  ```
  **只用 `mktemp` 而不配 `trap` 不算数**——随机后缀只防撞名，不会自己消失。
...
```bash
# 前置：MinIO 镜像是私有 GHCR package（D-036，按 digest 钉定），首次运行需先登录：
...
**当前 track（2026-09-08 拍板）**：**写入准入与索引一致性（IC-0…IC-15）**——2026-09-08 审计发现
...
追踪 `docs/tasks/consistency-ingest.md`，缺陷 `docs/tasks/bugs/open.md`（IC-BUG-1…IC-BUG-54，已关 37 条、撤销 1 条、未关 16 条：2 挡（7/40）/ 11 可推 / 3 拆半（IC-4a 关 6/9））。
...
**IC-3** ✅（续传落盘 + 孤儿分片清理，PR #105，六轮 review）→ **D-034** ✅（路径模板保留字 `{time}` 改名 `{submit_time}`、优先级对齐、保留字成文对齐三端，PR #106，六轮 review）→ **IC-4a（①③ + 死信表）已收官——(d) 只落了 `active=true` 标记位、强制解封归 IC-12 的验收；IC-4 ② 归 IC-4b；~~下一刀在 IC-4b / IC-15 之间选~~（IC-SEC-2 随 PR #109、IC-BUG-44/43 随 PR #108 收官）**
⚠️ **该「下一刀」已作废**（2026-09-24，A 基线审计后重排）：IC 残留项在「CI 绿灯可信 + 账本重判」
之前都不动。**当前执行顺序以 [`docs/tasks/active.md`](docs/tasks/active.md) 的「下一步」一节为准。**
~~→ 地基 IC-6/7 → 准入 IC-8…10 → 对账 IC-11…13 → 血缘 IC-14 → **tail 重做 IC-15**~~
⚠️ **该执行链同样已作废**（2026-09-24）——其中 IC-6/7、IC-11…13、IC-15 均在 `active.md`
「下一步」的**「明确不做」**清单里。
...
**路径模板保留字的权威清单在 `docs/design/contracts.md` V-3**（D-034 起三端对齐；`{time}` 已弃用为 `{submit_time}` 的别名，仍可用）。
