...
**IC-3** ✅（续传落盘 + 孤儿分片清理，PR #105，六轮 review）→ **D-034** ✅（路径模板保留字 `{time}` 改名 `{submit_time}`、优先级对齐、保留字成文对齐三端，PR #106，六轮 review）→ **IC-4a（①③ + 死信表）已收官——(d) 只落了 `active=true` 标记位、强制解封归 IC-12 的验收；IC-4 ② 归 IC-4b；下一刀在 IC-4b / IC-15 之间选（可并行；IC-SEC-2 随 PR #109、IC-BUG-44/43 随 PR #108 收官）**
