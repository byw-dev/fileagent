...
#### 其三：防抖 pending 表的有界性（第二轮返工 S-1 定稿）
...
- **被否决的第一版**（第一轮返工 R-1）：flush 后经固定容量通道发回收消息，满了就丢——
  ...
  复现：256 路径残留 192 条）。总量仍无上界，R-1 未完成，整套机制被 S-1 推翻。
- **定稿机制**：`fired` 标记（flush 完成后置位、Reset 时清除）+ **摊还扫描**——事件循环
  每处理完一个事件检查一次水位，超过阈值就把所有已 fired 的条目扫掉，并把阈值**几何式**
  抬高到 `2×len+64`（几何抬高是关键：逐事件扫描会让一次 N 文件的突发退化成 O(N²)）。
- **界，如实陈述（不是「不会泄漏」）**：稳态上界 ≈ `2 × 活跃（未 flush）路径数 + 64`；
  **一次突发之后若事件彻底停止，残留会停在当时的水位**（没有事件就没有扫描）——这是
  被那次突发规模限住的**有界残留**，不是无界增长。两者的区别必须写清，这正是第一版
  栽跟头的地方。
