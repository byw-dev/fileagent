...
- **本清单**：CC-1 file_deleted（PR #47 / D-017）、CC-2 queue_max_size（PR #48）、CC-6 离线兜底扫描（PR #51）、CC-5 错误响应 `request_id` + 未知参数拒绝（本 PR）。
...
`CC-1 ✅` → `CC-2 ✅` → `CC-6 ✅（离线兜底扫描）` → `CC-3 ⏸️ 推后（低价值）` → `CC-5 ✅（request_id + 未知参数）` → `CC-4 / CC-7（下一步）` → `CC-8/9（视使用价值）`
