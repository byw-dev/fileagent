...
> **已废弃的别名**，正式名是 **`{submit_time:…}`**（旧名仍可渲染，语义等价）。且 **`{submit_time}` /
> `{time}` 的裸形式与非时间类型引用已被三端一致拒绝**——因此本文那段「`{字段名:LDML格式}` —— 时间字段，
> 例如 `{time:yyyy/MM/dd}`」的说明**对保留字不再成立**（对普通字段名仍成立）。照抄本文示例会得到一个
> **必失败**的模板。**路径模板保留字的权威清单在
