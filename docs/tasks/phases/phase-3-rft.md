...
> ⚠️ **保留字已改名（2026-09-13，D-034 / PR #106）**：本文全篇示例里的 `{time:…}` 现为
> **已废弃的别名**，正式名是 **`{submit_time:…}`**（旧名仍可渲染，语义等价）。且 **`{submit_time}` /
> `{time}` 的裸形式与非时间类型引用已被三端一致拒绝**——本文第 633 行那段「`{字段名:LDML格式}`」
> 的说明对保留字不再成立。**路径模板保留字的权威清单在
> [`docs/design/contracts.md`](../../design/contracts.md) V-3**，本文未逐处回填（历史规格原文保留），
> 照本文实现前请先读 V-3。
