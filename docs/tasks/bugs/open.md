...
| **⚠️ IC-11（NATS）不解决它——2026-09-10 实测** | 曾被问「D-031 换成 NATS JetStream 后是不是就没这问题了」。**不是。** 在 dev MinIO 上临时挂 `notify_nats` target 实测，同一个键的载荷是：`"object":{"key":"ic19%2Fnested+dir%2F%E4%B8%AD+%E6%96%87.csv"}`——**与 webhook 完全相同的编码**。原因是编码发生在 MinIO **构造事件对象**时，不在传输层：`%2F` 位于 JSON 字符串字段**内部**，HTTP 与 NATS 都不会改写 JSON 字符串的内容。**因此本条与 IC-11 完全正交，不能等 IC-11 一起解决** |
| **⚠️ 别用顶层 `Key` 字段绕过** | 实测发现 NATS 载荷带一个**未编码**的顶层 `"Key":"data-sensor/ic19/nested dir/中 文.csv"`。**不要用它**：它是 `bucket/key` 拼接、且属 MinIO 私有信封字段（不在 S3 事件规范内，webhook 解析结构体 `minioS3Object` 也没有它）。正解仍是对 `s3.object.key` 做 `QueryUnescape` |
