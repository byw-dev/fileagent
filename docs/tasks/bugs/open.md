...
| IC-BUG-52 | `dry_run_limit` 收了、校验了、夹到 50 了，**然后从不下发**——`PushRuleCommand` 不带该字段，agent 硬编码上限 10，调用方请求 50 静默只得 10 | 🟡 P2 | controlplane + agent + proto |
...
| **后果** | CP 的 `file_entries.sha256` / `size_bytes` 可能**与 MinIO 里的对象不符，而两端都认为上传成功**。这不是「某次上传失败」，而是**索引与对象静默失配**——正是本 track 要消灭的那一类。另：`PutObject` 返回的 `info.ETag` 已经被取到并填进 `UploadResult.ETag`，它**精确描述了实际上传的字节**，却没有任何地方拿它与本地重算值比对——手边就有的校验信号被浪费了。声明给 MinIO 的 `size` 也来自①，而 **minio-go 会把 `*os.File` 包成 `io.NewSectionReader(f, offset, size)`**（`api-put-object-streaming.go:649` 的 `putObject`，因 `*os.File` 同时是 `io.ReaderAt` 与 `io.Seeker`）——**上传的字节区间被钉死在 `[offset, offset+size)`**。由此两半性质不同，**只有一半是静默的**：**文件增长 → 多出来的尾部根本不在区间内，静默丢失**（对象看起来正常、上传报成功）；**文件缩短 → 可读字节少于声明的 Content-Length，请求失败**（响亮，不会静默成功）。⚠️ **但这只是「大小单调变化」这一维的结论，不要据此以为只有增长危险**——**同尺寸原地改写**（长度不变、内容变）既不越界也不短读，**上传成功而 sha256 静默失配**，与增长同属静默一类。准确表述：*就大小单调变化而言，仅增长静默、缩短响亮失败；而同尺寸改写同样静默*。（同尺寸改写也正是 **IC-BUG-48** 的盲区之一，两条同源。）⚠️ minio-go 那段是**读 v7.0.91 源码得出的推断，未实测**；立卡时只核实了本仓库三次读的事实 |
...
| 字段 | 内容 |
|------|------|
| **根因** | CP 的 dry-run 接口**完整地接收并校验**了 `dry_run_limit`（`<=0` 补 10、`>50` 夹到 50），随后构造 `ServerMessage_PushRule` 时**根本没把它带上**——`PushRuleCommand` / `CollectionRule` 里不存在这个字段。agent 侧则是一个**硬编码常量** `defaultDryRunLimit = 10`，与请求无关 |
| **精确位置** | `controlplane/internal/api/handler/agents.go:616-621`（校验与夹取）、`:642-650`（构造 PushRule，未携带）；`agent/cmd/agent/main.go:1096`（`const defaultDryRunLimit = 10`）、`:1136`（`if count >= defaultDryRunLimit`）；`proto/v1/agent.proto` 全文无 dry-run limit 字段 |
| **后果** | 调用方（Web UI / SDK）请求 50 条预览，**永远只拿到 10 条，且没有任何提示**——没有截断标记、没有警告、没有错误。API 契约上这个参数看起来是生效的（它甚至被认真地夹取到上限），实际是个装饰。规则模板改错时，预览只看 10 条会让人误以为「匹配面就这么大」 |
| **发现经过** | **PR #107 二轮 review 中被顺带撞见**——起因是核对 `phase-3-rft.md` 的「已落地」措辞是否过强，该文 `:866` 要求 `config.toml` 新增 `[collection].dry_run_limit`（注释写明「上限 50」），核实后发现 agent 既无该配置项、也不接收下发值 |
| **所属模式** | **「参数收了不用」的第 4 个实例**（前三个：IC-BUG-5 续传状态、IC-BUG-11 tail 的 `file_offset`/`append_mode`、IC-BUG-38 的 `log.output`/`max_size_mb`/`max_backups`）。open.md 的模式归纳里已写「够立类了，下一轮归纳应正式收编」——**本条是第 4 个实例，收编的依据已经足够** |
| **修复** | 三选一，**建议 (a)**：(a) proto 增字段（只增不改，符合 `buf breaking` 约束）→ CP 下发 → agent 用下发值、无值时回落 10，并在结果里带「是否被截断」标记；(b) 若判定该参数不该存在，则 CP 侧直接删掉接收与夹取，避免契约撒谎；(c) 最小止血：CP 在 `dry_run_limit != 10` 时返回 warning 说明「该参数当前不生效」。**不接受维持现状**——一个被认真校验却永不生效的参数，比没有这个参数更糟 |
