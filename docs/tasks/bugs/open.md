...
> **计数（2026-09-17，PR #108 合并后 + PR #109 评审改判；2026-09-24 IC-4a 关闭 6/9）**：共 **54** 条 = **已关闭 37** + **已撤销 1**（IC-BUG-49，前提被实测证伪）+ **未关闭 16**（**2 条挡** 7/40 + **11 条可推** + **3 条拆半** 13、46 与 32）。**口径**：拆半计入「未关闭」（与 `active.md`、`consistency-ingest.md` 一致），因为功能缺口仍在。三张拆半卡片（13/46/32）的总览行均已带 ◐ 标记；分诊与判据以 `consistency-ingest.md` 的分诊表为准。42…45 来自 PR #100 的两轮 review，46…49 来自 IC-3 的六轮 review 与随后的 tail 设计讨论，**50 来自 tail 讨论中撞见的隐藏保留字（已随 PR #106 关闭）**，**51 是 IC-3 review 期间发现、当时按产品要求推后立卡的「三次独立读」**，**52 来自 PR #107 的多轮 review，53 来自 PR #108（IC-BUG-44/43）的 codex 复审**。
...
41 条里**有 30 条归得进三类成因**（M-1/M-2/M-3）。**⚠️ 这段归纳的时点是 2026-09-10、样本是当时的 41 条**——此后新增的 42…52 尚未归类，M-3 也仍未扫；**下一轮归纳需连同新增 10 条一起重做**，并正式收编「参数收了不用」（已有 IC-BUG-5/11/38 三个实例——⚠️ **IC-BUG-52 曾被误归入此类，经 PR #107 三轮 review 读码证伪，不要再把它算作第 4 例**）。M-1 与 M-2 的类扫描均已完成（结论见下方两小节），
...
| **✅ 已修（IC-4a，2026-09）** | **唯一一条失败路径，不按 4xx/5xx 分流**（dev 实测 MinIO 对 400/500 一视同仁）：处理失败 → 按事件身份（`bucket+key+sequencer`，webhook 请求头无事件 ID）在 **Redis 持久计数器**（键 `webhook:fail:<sha256>`，无 TTL，成功显式删除——进程内存计数在 CP 重启后归零，毒丸永不失效）上计数 → 未超上限（`WEBHOOK_FAIL_LIMIT`，默认 600 ≈ 30 分钟容忍窗口，大于预期故障时长 ÷ 3s）返回 5xx 让 MinIO 重投 → 超限落 **`webhook_dead_letters` 表**（迁移 000007）并返回 200 放行队头阻塞单队列。**谁 redrive**：运维（人工/脚本重放），规程见 `consistency-and-ingest.md` §3.6；死信表含完整重放载荷与 `last_error`。**(d)** ObjectRemoved 死信行 `active=true`——丢失的 delete 是「PG 有 / MinIO 无」，删除不推进 `object_keys.last_modified`，已封存分片永不解封，redrive 时必须强制置分片 `active`。**(c-2)** IC-2c 的解码失败「索引原值 + Warn」兜底改判为死信 + 5xx（进索引会写入无人匹配的 `storage_path`）。配置项 `WEBHOOK_FAIL_LIMIT`（默认 600，非 hardcode）；变异矩阵 7 项护栏各自被杀（`BUILD_OK` 验证）；live 证据：断 PG → 5xx + Redis 计数爬升 → 恢复 PG → 重投入索引、计数清零；毒丸 3 次失败后落表且 200，后续正常事件不被队头阻塞 |
...
| **🔴 刻意留下（IC-4a 未做，勿当成已闭环）** | ① **IC-4b**：`MakeBucket` 后 `SetBucketNotification` + 启动时幂等 ensure / ARN 可配置 / IC-BUG-7——本刀未动 ②。② **自动 redrive** 不存在：`webhook_dead_letters` 的重放是运维动作，无告警接线（`fail_count`/行数建议接监控）。③ **死信的 (d) 强制置分片 active 目前只是标记**（`active=true` 列）——IC-12 落地前无 `shard_state` 表可置位，redrive 流程须等 IC-12 后才能真正执行解封。④ **解码失败 now 死信 + 5xx**：若 MinIO 侧真出现持续解码失败载荷会重投至上限后落表，不会无限阻塞 |
...
| **`observed_at` 取值来源（2026-09-10 两次修订后定稿）** | 判据是「**这个值客户端能不能左右**」：`minio_event` ← 载荷的 `eventTime`（**MinIO 生成**）；`agent`/`api` ← **PostgreSQL 的 `now()`**（§3.5 坑 3 的硬约束，不是 CP 进程时钟）；`audit` ← **列举那一刻**（不是写入事务的 `now()`——否则陈旧列举会覆盖更新数据并污染封存判据，见 §3.4 实测）。**⚠️ 绝不采信 `UploadResult.uploaded_at`**（agent 提供，报 `2099` 即可永久冻结该行，此后连 IC-13 对账都写不进去）。**⚠️ 也不要因此把 `minio_event` 一并改成 CP 受理时刻**——中间版本曾如此定案，评审用真 PG 证伪：四源统一取受理时刻会让 `>=` 谓词**恒真**、闸门变摆设，而 `IndexUpload` 写的 `size_bytes`/`status`/`uploaded_at`/`etag` 都是非空值、`COALESCE` 一列都保护不到，等于本条根本没修。**完整 SQL 守卫与 NULL 语义见 `consistency-and-ingest.md` §3.4** |
...
| **后果** | 凡是带层级的对象键（正常情况）索引值都与真实键不符。①预签名下载用 `storage_path` 作 key，必然 404；②`fileNameFromPath(objectKey)`（`indexer.go:547`）把整个编码键当文件名（dev 实证：`file_name = 'Miru%2Fkobe%2Fkobe_006.csv'`）；③`Classifier.Classify` 的 `filepath.Match` / `filepath.Base` 看不到分隔符（`classifier.go:36-45`），文件类型判定失效；④`IndexDeletion` 的精确匹配软删**永不命中**，删掉的对象留下永久幽灵行；⑤IC-6 之后 `object_keys` 的幂等键与对账会把这些行全判成幽灵。**当前 `file_entries` 的唯一写入者就是这条路径**，所以影响是全量的。**⚠️ 2026-09-10 PR #96 评审更正**：初版把「path_var 反解拿不到分隔符，打标失效」列为后果——**在 webhook 路径上不成立**，`applyPathVarTags` 只在 `indexer.go:221`（`HandleUploadResult`，agent 路径）被调用，而 agent 侧的 storage path 从来没被编码过。打标要到 IC-2a 之后才进这张清单 |
| **修复** | ✅ **已随 IC-2c 修（PR #96）**：在 `MinioEventHandler.Handle` 解析后对 key 做一次 `url.QueryUnescape`（S3 事件用的是 `+`-as-space 的 query 编码，不是 path 编码），失败时退回原值并告警——**丢事件比索引一个怪键更糟**（丢的 create 会让 MinIO 里躺着一个索引永远不知道的对象）。单测 10 例（8 个表驱动子用例：带层级 / `+`-空格 / `%20`-空格 / 中文 / 字面加号 / 模板形状键 / **字面 `%2F` 文本双重转义** + 删除路径 + 非法编码退回；`%2F` 那例是 PR #96 评审补的，专门拦「未来好心多解一次码」）。**变异矩阵验证过能证伪**（数字是实跑出来的，不是估的）：不解码 → **8** 例失败（7 个子用例 **+ 删除路径**；`flat_key` 不失败是对的，扁平键本来就无需解码）；换 `PathUnescape` → **3** 例失败（`+`-空格、中文 **+ 删除路径**）；解码失败不退回原值 → 非法编码用例失败。**两次变异里删除路径都是更要紧的那一击**——它是静默 no-op，不像 `storage_path` 长得怪那样一眼可见 |
...
| **⚠️ 盲区（PR #108 codex 评审补充）** | 写入方**倒拨 mtime**（`touch -t` / `os.Chtimes`）可绕过 close_wait 的静默判据——`time.Since(mtime)` 只看墙钟，被倒拨的「写入中」文件会被当作「已静默」而**上传截断版本**（F2「绝不上传写入中的文件」从 PR #100 起就完全依赖 mtime 判据，这是既有性质，非 IC-BUG-44/43 引入）。归属本卡片的 mtime 边界，与 ①② 同根：判据依赖可被写入方操纵的元数据。来源：PR #108（IC-BUG-44/43）的 codex 评审 |
...
## IC-BUG-51 — sha256 / size / 上传字节来自三次独立读，可描述不同版本 🟠 P1

> **定级**：立卡时定 🟡 P2，**经 PR #107 review 上调为 🟠 P1**——静默的索引↔对象失配正是本 track 要消灭的那一类，且它是 IC-11…13 对账的硬前置；触发条件的稀有性不折抵后果的性质。
...
| **精确位置** | `agent/internal/uploader/uploader.go:168`（stat）、`:190`（`fileSHA256`）、`:206`（版本快照）、`:238-239`（`result.SHA256` / `result.SizeBytes` 赋值）、`:285-297`（第三次读 + PutObject）；multipart 路径共用②的 `sha`，同样成立 |
| **后果** | CP 的 `file_entries.sha256` / `size_bytes` 可能**与 MinIO 里的对象不符，而两端都认为上传成功**。这不是「某次上传失败」，而是**索引与对象静默失配**——正是本 track 要消灭的那一类。另：`PutObject` 返回的 `info.ETag` 已经被取到并填进 `UploadResult.ETag`，它**精确描述了实际上传的字节**，却没有任何地方拿它与本地重算值比对——手边就有的校验信号被浪费了。声明给 MinIO 的 `size` 也来自①，而 **minio-go 会把 `*os.File` 包成 `io.NewSectionReader(f, offset, size)`**（`api-put-object-streaming.go:649` 的 `putObject`，因 `*os.File` 同时是 `io.ReaderAt` 与 `io.Seeker`）——**上传的字节区间被钉死在 `[offset, offset+size)`**。由此两半性质不同，**只有一半是静默的**：**文件增长 → 多出来的尾部根本不在区间内，静默丢失**（对象看起来正常、上传报成功）；**文件缩短 → 可读字节少于声明的 Content-Length，请求失败**（响亮，不会静默成功）。⚠️ **这条结论的适用范围很窄，不要当成完整分类**——它只覆盖「**文件始终是同一个 inode、且只考察相对首次 stat 的尺寸单调变化**」。在该范围内：仅**增长**静默（尾部丢失、上传仍报成功），**缩短**响亮失败。**范围之外至少还有三种同样静默的情形**（PR #107 三轮 review 补充）：①**同尺寸原地改写**——长度不变、内容变，既不越界也不短读；②**truncate 后又在读取期间涨回原尺寸**——可能读到跨版本的混合字节且上传成功；③**路径被同尺寸的新 inode 原子替换**（写临时文件再 rename，日志轮转的常见做法）——不属于「原地改写」，但三次读会打开不同的文件。另有 ④ `offset>0` 的 tail 路径本身就是「哈希全文、只传后缀」，**当前被 executor fail-closed 挡着**（IC-BUG-46），不是现在可达的生产场景，但 IC-15 做 tail 时必须一并解决。（①③ 也正是 **IC-BUG-48** 的盲区，两条同源。）⚠️ minio-go 那段是**读 v7.0.91 源码得出的推断，未实测**；立卡时只核实了本仓库三次读的事实 |
...
## IC-BUG-52 — `dry_run_limit` 收了不用：请求 50 静默只得 10 🟡 P2
...
| **修复** | 三选一，**建议 (a)**：(a) proto 增字段（只增不改，符合 `buf breaking` 约束——**三轮 review 已确认相容**，且在 `DryRunResult` 增 `truncated`/`total` 同样相容）→ CP 下发 → agent 用下发值、无值时回落 10，并在结果里带「是否被截断」标记；(b) 若判定该参数不该存在，则 CP 侧直接删掉接收与夹取，避免契约撒谎；(c) 最小止血：CP 在 `dry_run_limit != 10` 时返回 warning 说明「该参数当前不生效」。**不接受维持现状**——一个被认真校验却永不生效的参数，比没有这个参数更糟 |
| **验收** | 请求 `dry_run_limit=50` 且 base_path 下有 >10 个匹配文件 → 预览返回 50 条（或返回 10 条但带明确的截断标记）；变异：agent 端改回硬编码 10，该用例必须变红 |
| **归属** | 未排期。**不阻塞任何在途刀**；宜与 `phase-3-rft.md` 遗留项一并清理 |
...
| 字段 | 内容 |
|------|------|
| **根因** | watcher 侧把事件送进内存 channel（`emitBlocking` 返回 true）就算「处理完毕」并记入 `seen`——但 `emitBlocking=true` **只证明事件进了 channel，不证明下游已持久入队**。消费端 `runWatcher` 收到事件后调 `submitFile`，其中 `exec.Submit` 失败（SQLite busy timeout、磁盘 I/O 等）**只打一条 Warn**。此后该文件若不再变化（日志轮转后的旧文件、一次性落地的文件），就再没有任何事件触发，**当前 watcher 生命周期内永不被采集——没有持久重试状态，只有日志**。安全网重扫（IC-BUG-44）对它**帮不上忙**：重扫复用的是同一个 `seen`，初扫交付后 Submit 失败的文件已被标记，**重扫会一直跳过，只有重启/重建 watcher（新 seen map）才能恢复**。`IsProcessed` 出错后代码仍会继续调 `Submit`，它不是本卡所述漏采的直接原因，主要风险是**重复提交**（与 IC-BUG-42 同向） |
...
| **修复** | 长期形态是**交付→持久化闭环**：下游持久入队后向 watcher 回传确认（FileEvent 携带 ack 回调或独立的 ack channel），确认到达才记 seen；需处理 ack 丢失/顺序/泄漏，并与 executor 的重试退避层对齐（「任务入队失败」与「任务执行失败」是两层，ack 语义要定清楚）——跨 watcher 与 main 的边界，**独立一刀**。在闭环落地前，初扫/close_wait 路径的文件没有运行期兜底（安全网重扫被同一 seen 挡住），唯一恢复手段是重启/重建 watcher；也可考虑先修 submitFile 的静默失败本身（失败可见化/重试/阻塞消费），但同样动下游语义，宜单独评估 |
| **验收** | 构造 Submit 失败（mock executor 返回错误）→ 文件在重启后的初扫中必须被重新提交；ack 闭环落地后：Submit 失败的文件在确认缺失时必须被重新交付，且恰一次仲裁（PR #108 F3）不因 ack 机制破坏 |
