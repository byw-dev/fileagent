...
| **MT-2** | CP indexer + API | 打标引擎第一斩：规则 `metadata.file_type` + `static_tags`（source=`rule_static`）在 `UploadResult` 索引时落 `file_tags`（幂等 on-conflict）；classifier 优先级=声明类型>glob；文件响应带 `tags`；`GET /files` 增可重复 `tag` 参数（扩 `RejectUnknownQuery`，**保持 cursor 分页 V-2**） | **核心闭环**：建规则声明 static_tags → 上传 → 文件带标签 → `?tag=k:v` 能筛出；既有 glob 部署行为不变 | ⬜ |
...
| **MT-4** | CP API | 词表 CRUD（`/api/v1/tag-keys` + `/{key}/values`）；待确认 `GET /pending-tag-values` + `/{id}/approve\|merge\|reject`；**手动/批量打标**：`PUT /files/{id}/tags` + `POST /files/batch-tag`（source=`manual`，写 `tag_audit`，未登记值同样入队）。全部 super_admin 写 | merge 触发回溯改写 + 审计；批量打标按 `GET /files` 同款谓词圈选（服务历史数据人工补标） | ⬜ |
