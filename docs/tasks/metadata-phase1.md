...
| **MT-6** | webui | `7a` 规则表单「元数据」步骤 · `7b` 文件页 faceted 筛选 + 批量选中打标 · `7c` 设置·标签词表 · `7d` 待确认队列。**用 WR-1 token「够用一致」，不追 mockup 像素** | 四屏可用即收；live-e2e 走通 7a→上传→7b 筛选 | 🔄 **拆分**（按屏）：**MT-6a** `services/tags.ts` + `7c` 标签词表（`Settings/TagKeys`，key/value CRUD，super_admin 写）🔄 PR，**live 已验证**（真 CP+浏览器：列表/取值抽屉/新增取值）；**MT-6b** `7d` 待确认队列（list/approve/merge/reject）⬜；**MT-6c** `7b` 文件页 faceted 筛选 + 批量打标 ⬜；**MT-6d** `7a` 规则表单元数据步骤 ⬜ |
