...
**未关闭 23 条：9 条挡、11 条可推、3 条拆半**（12/13/31 各关掉一半）。

**这 23 条里没有一条再挡着「数据面能不能用」**——那三条判据（落得进 / 记得准 / 不倒退）在
IC-1 + IC-2c + IC-2a + PR #97 之后已经全部满足，并有 live 证据。**剩下的「挡」是挡着「能不能扛住故障」**
（重试、断连、崩溃恢复、事件丢失），不是挡着「能不能跑通」。下一刀选谁，用的是这个新口径。
