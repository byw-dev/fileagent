...
**49 条中已关闭 30 条**：IC-1 关 1/3/4/16/17/18/22/23，IC-SEC-1 关 24/25，IC-2c 关 19，
...
**IC-BUG-50/51 是 tail 设计讨论与 IC-3 review 的副产品**：**50**（`time` 是未声明的隐藏保留字、优先级与同体系变量相反、名字误导——**已随 D-034 / PR #106 关闭**；⚠️ **它不是被任何测试发现的**，是讨论 tail 时顺手读 `pkg/trollsift/parser_test.go` 撞见的，隐性契约不会让任何用例变红）、**51**（`sha256` / `size_bytes` / 实际上传字节**来自三次独立的文件读**，可分别描述文件的不同版本，而手边的 `PutObject` ETag 无人复核——🟡 P2，但**是 IC-11…13 分片对账的硬前置**：对账的全部前提就是「DB 记的 sha256/size 可信」，不修则好对象被判不一致、坏对象被判一致；宜与 **IC-BUG-48** 同刀，共用 ETag 校验基建）。
