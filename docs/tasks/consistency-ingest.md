...
> **缺陷清单**：[`bugs/open.md`](bugs/open.md) IC-BUG-1…IC-BUG-35
...
| 缺陷 | **`IC-BUG-1` … `IC-BUG-35`** | 见 [`bugs/open.md`](bugs/open.md)，沿用 `T3-5-BUG-x` 的既有形状 |
...
35 条中已关闭 11 条（IC-1 关 1/3/4/16/17/18/22/23，IC-SEC-1 关 24/25，**IC-2c 关 19**）。
**未关闭 24 条：15 条挡、7 条可推、2 条拆半。**（IC-BUG-35 是 IC-2c 的 live 验收顺带发现的部署脚本缺陷，见下表末行）
...
| 35 | ⬜ 可推（正交） | 未排期，宜并入 IC-4 | 与写入准入/一致性无关，但 `init-minio.sh` 默认 access key 超 MinIO 20 字符上限、脚本必 `exit 1`，**挡着任何人从零复现 live 验收**。IC-4 同样要动这个脚本，合并处理最省 |
...
>
> **止血阶段顺序**：IC-1 ✅ → **IC-2c** ✅（PR #96）→ **IC-2a**（下一刀）→
> （IC-2b / IC-3 / IC-4 / IC-5 / IC-SEC-2 可并行）。
> **IC-2c 必须早于 IC-2a**（不是「宜早」，是硬序：见 IC-2c 一栏）——**该硬序已满足，IC-2a 可以开工**。
>
