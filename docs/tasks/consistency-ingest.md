...
| ~~F~~ | ~~`observed_at` 的时钟归属~~ | ~~IC-2a~~ | ✅ **已定（2026-09-10，两次修订后定稿）：按 source 取各自最可信且客户端左右不了的时刻**——`minio_event` ← 载荷的 `eventTime`（MinIO 生成）；`agent`/`api`/`audit` ← **PostgreSQL 的 `now()`**（不是 CP 进程时钟，见 §3.5 坑 3）。删除/创建因果另用事件的 `sequencer`（`event_seq`，仅在 `observed_at` 相等时决胜）。**中间曾定「四源统一取 CP 受理时刻」，评审用真 PG 证伪后撤回**（谓词恒真、闸门变摆设）。完整模型、守卫 SQL 与实测数据见 `consistency-and-ingest.md` §3.4 |
