...
| ID | 模块 | 内容 | 验收要点 | 状态 |
|----|------|------|----------|------|
| **IC-11** | deploy + CP | **事件传输 webhook → NATS JetStream**（**D-031**，对账阶段前置）。**⚠️ 换传输 = 换消费入口，以下已落地的逻辑必须一并搬进 JetStream consumer，否则被静默回退**：**IC-2c 的 `url.QueryUnescape`**（D-031 扫描表把 IC-BUG-19 记为「❌ 不解决」，含义是「IC-11 不修它」，**不是「IC-11 不用管它」**）、IC-4 ② 的 bucket 通知 ensure（ARN 改为 NATS target）、以及 IC-4 ① 的失败语义（由 HTTP 5xx 换成 nak/不 ack）。① MinIO 改配 `notify_nats` + `jetstream=on`；② CP 改用 JetStream durable consumer，**处理成功才 ack**；③ 鉴权载体由共享密钥换为 NATS creds/nkey/TLS（D-014 作废）；④ **stream sequence 只喂 `shard_state.last_event_seq`（链路自证），不得用来填 `observed_at`**——`observed_at` 是 `TIMESTAMPTZ` 而 sequence 是 `uint64`，且它只对 `minio_event` 一路单调、与另外三个 source 不可比。**D-031 原文「可直接用作 `observed_at` 来源」已就地更正**，见该决策的「更正」一节 | 停 CP 期间写入的对象，CP 重启后从 stream 续读并补齐索引；能按序号重放一段历史 | ⬜ |
...
| **F** | **`observed_at` 的时钟归属**：四个 source（`agent`/`api`/`minio_event`/`audit`）分属**三个互不同步的时钟**（agent / MinIO / CP）。全用 CP 受理时刻则单一时钟、且天然不受客户端控制，但**事件乱序到达时会把「删除」判成早于「创建」**（软删除的时间围栏失效）；用各自的事件时间则顺序正确但跨时钟不可比。**IC-2a ⑤ 的钳制是止血，不是答案** | **IC-2a** | 倾向「CP 受理时刻为主 + 事件时间单独存列供对账用」，需实测乱序场景后定 |
