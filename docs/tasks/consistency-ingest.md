...
**IC-5（PR #100）、IC-2b（PR #103）、IC-3（PR #105）与 D-034（PR #106）均已收官。下一刀在 IC-4 / IC-15 之间选，彼此可并行**（**IC-SEC-2 已随 PR #109 收官、IC-BUG-44/43 已随 PR #108 收官**，均不再是候选），
之后才是有时间窗口的地基 IC-6/7。
...
**54 条中已关闭 35 条**（2026-09-17 更新：PR #108 新立 IC-BUG-53；2026-09-13：IC-3 另关 47、D-034/PR #106 关 50；49 已撤销）：IC-1 关 1/3/4/16/17/18/22/23，IC-SEC-1 关 24/25，IC-2c 关 19，
**IC-2a（PR #98）关 2/8/28/29/33**，**PR #97 关 35/36**，**IC-5（PR #100）关 10/11/34/37 并补上 12 的超时半边**，
**IC-2b（PR #103）关 20/21/30**，**IC-3（PR #105）关 5/39/47 并挡掉 46 的出血半边**（ILM 兜底一条见卡片——当前 MinIO 全代系不实现该 action，脚本照写并响亮告警），**D-034（PR #106）关 50**，**PR #108（IC-BUG-44/43）关 43/44**，**IC-SEC-2（PR #109）关 26/27**。
**未关闭 18 条：4 条挡、11 条可推、3 条拆半**（13、**46** 与 **32**；**12 与 31 均已两半齐全**；32 的拆半判于 PR #109——空闲路径随 IC-SEC-2 关闭、积压下送达保证未排期）。
...
「挡」的 5 条原是 5/6/7/9/40（**其中 6/7 是「挡（收窄）」，仍算挡，不要当成可推**）——**5 已随 IC-3 关闭**，**46 曾短暂计入「挡」，现已随 IC-3 挡掉出血半边、转为拆半**，故「挡」仍为 **6/7/9/40**。
