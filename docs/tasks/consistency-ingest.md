...
> **缺陷清单**：[`bugs/open.md`](bugs/open.md) IC-BUG-1…IC-BUG-54
> **来由**（2026-09-08）：产品提出两条此前不成立的前提——必须允许 ETL 等非 Agent 进程写入并记录
> tags 与血缘；对象量级为千万/年、3–5 年上亿。据此审计发现 **Agent 数据面从未端到端跑通过**。

---

> # ⚠️ 本文件的执行顺序已于 2026-09-24 整体作废
>
> A 基线审计（PR #114/#115/#116）之后，**IC 残留项在「CI 绿灯可信 + 账本重判」之前都不动**。
> **当前执行顺序以 [`active.md`](active.md) 的「下一步」一节为准。**
>
> 本文内所有「下一刀是…」「应尽快推进…」「宜早不宜迟」之类的**现行语气指令均已失效**
> （下文已就地标注，但若有遗漏，**一律以 `active.md` 为准**）。
> 技术内容（缺陷分析、设计取舍、验收标准）仍然有效——失效的是**排期**。
...
**IC-5（PR #100）、IC-2b（PR #103）、IC-3（PR #105）、D-034（PR #106）与 IC-4a（2026-09）均已收官。~~下一刀在 IC-4b（② 收尾）/ IC-15 之间选~~ ⚠️ **已作废**（2026-09-24，A 基线审计后重排）——
IC 残留项在「CI 绿灯可信 + 账本重判」之前都不动，**当前顺序以 [`active.md`](active.md) 的「下一步」一节为准**（**IC-SEC-2 已随 PR #109 收官、IC-BUG-44/43 已随 PR #108 收官、IC-4a 已收官**，均不再是候选），
~~之后才是有时间窗口的地基 IC-6/7。~~ ⚠️ **排期已作废**，见顶部 banner。
**这刀各做什么，看下方「任务清单」里对应的行**（每行都有子项与验收），不要只看名字猜。
...
3. **地基阶段（IC-6/IC-7）有时间窗口。** 按 `system-design.md` §6.7 换算约 2600 万对象/年，
   ...
   不要让它排在准入 / 对账之后。~~
   ⚠️ **2026-09-24 限定**：上述**风险本身仍然成立**（它是关于未来的条件命题），但
   **时钟尚未开始走**——系统未发布、无数据累积、无存量迁移负担，因此**「应尽快推进」已作废**。
   待真实数据开始累积时重估。见 [`active.md`](active.md)「明确不做」。
...
| 类别 | 编号 | 说明 |
|---|---|---|
| 任务 | **`IC-0` … `IC-15`** | Ingest & Consistency，对应 `consistency-and-ingest.md`。按依赖顺序编号 |
| 缺陷 | **`IC-BUG-1` … `IC-BUG-49`** | 见 [`bugs/open.md`](bugs/open.md)，沿用 `T3-5-BUG-x` 的既有形状。**新编号接着往后取**，不要以为序号只到某个旧上限 |
...
> ### ⚠️ 这本账的可信度边界（2026-09-13，PR #107 三轮 review 后立）
>
> **事实性表述（行号、计数、某处代码是否存在）可信度高；归纳性表述（缺陷分类、
> 「只有 N 种情形」这类穷尽声明）可信度低。** 这不是谦辞，是 PR #107 三轮 review 的实测结论：
>
> - **两条我自己写的断言被读码证伪并撤回**：IC-BUG-52 的「参数收了不用」
>   （CP 在响应端确实按该参数裁剪，1–10 生效）、`phase-3-rft.md` 的「交付物均已存在」
>   （`[collection].dry_run_limit` 至今未实现）。两条的共同成因是**只验证支持自己结论的那一半代码就停手**。
> - **IC-BUG-51 的危险面描述连续三轮被指出「过窄」**：一轮「只有增长危险」→ 二轮补成三分法
>   → 三轮又发现漏了 inode 替换、truncate 后涨回、tail 全文哈希三种。**每次「精确化」都在缩小危险面。**
>   最终改为**显式限定适用范围 + 举例**，不再声称穷尽。
> - **第四轮被主动放弃**，理由：三轮的走向是「事实错误 → 表述过窄 → 仍是表述过窄」，**没有收敛**，
>   且剩余发现全部落在归纳文字而非实现。继续拉轮次大概率还能挑出第四处，边际价值已低于成本。
>
> **给后人的操作建议**：要依据本账本的某条**分类或穷尽声明**做决策（例如「这属于 M-2 类」
> 「失败写法只有两种」「只有增长才静默」），**先回代码核对**；要依据**行号、计数、某条缺陷是否存在**，
> 可以直接用——那些都经过程序化重算或逐行核对。

**54 条中已关闭 37 条**（2026-09-24 更新：**IC-4a 关闭 6/9**；2026-09-17：PR #108 新立 IC-BUG-53；2026-09-13：IC-3 另关 47、D-034/PR #106 关 50；49 已撤销）：IC-1 关 1/3/4/16/17/18/22/23，IC-SEC-1 关 24/25，IC-2c 关 19,
**IC-2a（PR #98）关 2/8/28/29/33**，**PR #97 关 35/36**，**IC-5（PR #100）关 10/11/34/37 并补上 12 的超时半边**,
**IC-2b（PR #103）关 20/21/30**，**IC-3（PR #105）关 5/39/47 并挡掉 46 的出血半边**（ILM 兜底一条见卡片——当前 MinIO 全代系不实现该 action，脚本照写并响亮告警），**D-034（PR #106）关 50**，**PR #108（IC-BUG-44/43）关 43/44**，**IC-SEC-2（PR #109）关 26/27**，**IC-4a 关 6/9**（webhook 失败语义 + 毒丸 + 死信表 + `queue_dir` 持久卷；**IC-BUG-7 仍开着，归 IC-4b**；**IC-BUG-6 的 (d) 只落了 `active=true` 标记位——强制解封 ObjectRemoved 死信对应分片是 IC-12 的验收项与依赖，IC-4a 不以 (d) 完成收官**）。
**未关闭 16 条：2 条挡（7/40）、11 条可推、3 条拆半**（13、**46** 与 **32**；**12 与 31 均已两半齐全**；32 的拆半判于 PR #109——空闲路径随 IC-SEC-2 关闭、积压下送达保证未排期）。
**IC-BUG-46 是本 track 第二例「先挡出血、再单独实现」的拆半**（前一例是 12/31 的上报与结构半边）：挡掉半边随 IC-3（CP 422 + agent 闸门 + UI 禁用选项并说明原因），**正确实现归 IC-15**。⚠️ **它仍然是一个功能缺口而非隐患**——tail 现在处于不可用状态，账面上从「挡」移出只是因为出血已止，**不代表该模式可用**。
**IC-2b（PR #103）关 20/21/30 并补齐 31 的结构半边**——重判时点名的两条产品语义缺口至此**全部关闭**
（10 随 IC-5，21 随 IC-2b）。
「挡」的 5 条原是 5/6/7/9/40（**其中 6/7 是「挡（收窄）」，仍算挡，不要当成可推**）——**5 已随 IC-3 关闭**，**6/9 已随 IC-4a 关闭**，**46 曾短暂计入「挡」，现已随 IC-3 挡掉出血半边、转为拆半**，故「挡」仍为 **7/40**。
**IC-BUG-46…49 是 IC-3 的五轮 review 与随后的 tail 设计讨论新立的**：**46**（`append_mode=tail` 的 `singlePartUpload` 把只含增量的内容整体替换对象，**静默丢数据**，🔴 P0——**IC-3 先 fail-closed 挡掉**、正确实现见 **IC-15**；⚠️ 挡掉的理由**不是「当前无消费方」**，而是一个已对外提供的模式会静默丢数据就必须先停）、**47**（实时 fsnotify 路径仍用非阻塞 `emit` 丢事件——**IC-5 的 F1 只修了初始扫描那一半**，随 IC-3 修）、**48**（内容身份靠 `mtime+size` 推断而非 ETag 验证，归 IC-15，一并关掉同秒同尺寸改写与保留 mtime 原地重建两个盲区）、**49**（追加后「新版本对象 vs 覆盖」语义未定义——**❌ 已于 2026-09-13 撤销**：其前提「`dest_path_template` 带不带 `{time}` 隐式决定版本/覆盖」被实测证伪，其中成立的那半个产品问题拆入 [`backlog.md`](backlog.md)「同一文件重复采集的覆盖语义」）。

**IC-BUG-50/51 是 tail 设计讨论与 IC-3 review 的副产品**：**50**（`time` 是未声明的隐藏保留字、优先级与同体系变量相反、名字误导——**已随 D-034 / PR #106 关闭**；⚠️ **它不是被任何测试发现的**，是讨论 tail 时顺手读 `pkg/trollsift/parser_test.go` 撞见的，隐性契约不会让任何用例变红）、**51**（`sha256` / `size_bytes` / 实际上传字节**来自三次独立的文件读**，可分别描述文件的不同版本，而手边的 `PutObject` ETag 无人复核——🟠 P1（立卡时定 P2，**经 PR #107 review 上调**：静默的索引↔对象失配正是本 track 要消灭的那一类，且它**是 IC-11…13 分片对账的硬前置**：对账的全部前提就是「DB 记的 sha256/size 可信」，不修则好对象被判不一致、坏对象被判一致）；宜与 **IC-BUG-48** 同刀，共用 ETag 校验基建。**两半性质不同、只有一半静默**：文件在三次读之间**增长**则尾部静默丢失（上传仍报成功），**缩短**则请求响亮失败——危险的是增长那一半。
**IC-BUG-42…45 是 PR #100 两轮 code review 新立的**（⚠️ **本段是立卡当时的分诊快照**：**43/44 已随 PR #108 关闭**，下面「四条均判为可推」只对 42/45 仍然成立），四条当时均判为可推并写明了刻意不修的理由，见各自卡片：
...
**这些可推的条目里没有一条再挡着「首次写入能否落地并记准」**——`file_entries` 有且只有一行、富字段不被清空、
...
> **IC-BUG-21 已随 IC-2b（PR #103）关闭**——至此重判时点名的**两条产品语义缺口全部闭合**。
> 剩下的「挡」都是容错性质（分片泄漏、webhook 可靠性、诊断成本），没有「稳态就错」的了。
...
| 6 | ✅ **已关闭** | **IC-4a ①（2026-09）** | 失败语义 + 毒丸 + 死信表落地（live 证据见 bugs/open.md 卡片）。⚠️ IC-4b ②（bucket 通知 ensure / ARN 可配置 / IC-BUG-7）仍未做——非 agent 写入的索引入口仍只有已订阅的桶 |
| 7 | 🟠 挡（收窄）| IC-4b ② | IC-2a 后 agent 路径有 ack 兜底，但非 agent 写入只有 webhook 一条命；IC-4a 关了 ①，本条（通知注册缺失）归 IC-4b |
| 8 | ✅ **已关闭** | **IC-2a ⑤（PR #98）** | **不与 2 同刀即数据损坏**——webhook 晚到清空富字段 |
| 9 | ✅ **已关闭** | **IC-4a ③（2026-09）** | `queue_dir` 迁至持久卷（`/data/minio-webhook-queue`）+ 脚本生效值校验；live 证据：停 CP → 写对象 → 重启 MinIO 容器 → 事件从持久队列投递、`file_entries` 补齐。**⚠️ IC-11 换 `notify_nats` 后该参数同样存在，不得回退**（D-031「全量扫描结论」） |
...
| 20 | ✅ **已关闭** | **IC-2b ①（PR #103）** | 新建指向新桶的规则 → 最长约 50 分钟持续 403，重试耗尽不自愈 |
| 21 | ✅ **已关闭** | **IC-2b ②（PR #103）** | 整桶 policy 后错键不再被 403 挡住，退化成**静默写错位置** |
| 28 | ✅ **已关闭** | **IC-2a ⑧（PR #98）** | 每文件一次 `Send` + `Register` 按 key 覆盖 + gRPC 无 recovery = **CP 进程崩溃** |
| 29 | ✅ **已关闭** | **IC-2a ⑦（PR #98）** | 上报活过来即可利用：借他人规则元数据打标，污染 6c 词表 |
| 30 | ✅ **已关闭** | **IC-2b ③（PR #103，改快照形态）** | 断连期间删掉的规则 agent 继续跑，**无界**直到进程重启 |
| 31 | ✅ **已关闭（两半齐全）** | **IC-2a ④（PR #98）+ IC-2b ④（PR #103）** | ack 半边随 IC-2a；结构半边（`SendCh` 满即静默丢弃 + `Connect` 在消费者启动前入队）随 IC-2b。**实测**：逐条 push 形态下 41 条消息对 32 缓冲，5/5 丢 8 条规则 + 凭据，故 ③ 改快照把 41 压成 1 |
...
| 26 | ✅ **已关闭** | **IC-SEC-2 ①（PR #109）** | 单组织 + 管理员鉴权下是越权面而非可利用漏洞；⚠️ 首版谓词误传规则行自身 `agent_id` 而恒真，评审抓出后改用 URL agent |
| 27 | ✅ **已关闭** | **IC-SEC-2 ②（PR #109）** | 能力型随机 ID，需 30s 内猜中 UUIDv4 |
| 32 | ◐ 拆半 | IC-SEC-2 | 稳态下命令实际能送达（评审实测证伪了初版「约半数丢失」）；**PR #109 拆半**——空闲路径确定性成立并有契约测试，积压下送达是尽力而为（`stream.Send`=入队非上 wire，收尾不等对端开窗），关闭路径=应用层 ACK，未排期 |
...
- **5 / 20 / 21 / 30 / 34（IC-3 / IC-2b / IC-5）**：**5 已随 IC-3（分片续传落盘 + 终态 Abort）关闭**；
  其余判定不变。这些是真正的「扛不住故障」：
...
> ⚠️ **「内容」列是开工前的基线快照**：它描述的是该刀**动手之前**代码的样子，用来说明要改什么。
> **已标 ✅ 的行，其「内容」列里的「当前 / 现在 / 从不 / 只有」一律指开工前，不是现状**——
> 判断现状请看「状态」列与 `bugs/open.md` 的卡片。（PR #109 评审发现多处被误读为现状，见 **IC-BUG-54**。）
...
| **IC-SEC-2** | CP | **归属与并发**（IC-BUG-26 + IC-BUG-27 + IC-BUG-28 已移入 IC-2a ⑧ + IC-BUG-32）。① `DeleteCollectionRule` 加 `AND agent_id AND org_id`，`:execrows`=0 → 404，**并让 `DeleteRule` 从 DB 读回真实 `agent_id` 再发 cancel**（当前发给 URL 里的 agent，是 IC-BUG-29 第 6 条路径的另一半）；② `dirstore.Store` 照抄 IC-SEC-1 给 `dryrun.Store` 的形状记收件人（`Register(reqID, agentID)` + `Deliver` 比对）；③ `Revoke` 的 `Send`→`Disconnect` 加**有界**短窗（≤1s）等发送确认，超时直接切——上限必须是硬的，不能等一个不读流的 agent | 用 agent A 的路径删 B 的 rule 返回 404 且 B 的规则仍在、**且不发出任何 cancel**（注意：① 前半落地后「cancel 发错收件人」在合法路径上已不可复现——URL agent 必然等于规则属主，所以**不要写「cancel 发给 B 而非 A」这种无法证伪的验收**；「从 DB 读回 `agent_id`」保留为纵深防御，用单测直接断言 dispatch 入参来自 DB 行而非 `c.Param`）；agent A 对发给 B 的 `request_id` 投递被丢弃且合法目录列举仍送达；bufconn：正常 agent 被吊销时先收到 `RevokeCommand` 流才断，不读流的 agent 在上限内仍被切断 | ✅ PR #109（四轮 codex 复审 + 一轮全新复核；**IC-BUG-32 只关空闲路径半边**，积压下送达保证如实留为未关半边）|
| **IC-2b** | CP + agent | **下发链路鲁棒性**（IC-BUG-20 + IC-BUG-21 + IC-BUG-30 + IC-BUG-31 的结构半边）。① **凭据随 bucket 集合变化补发**（修 IC-BUG-20）：**CP 侧** `DispatchRule` 成功后若该 Agent 的 bucket 集合变了就重推一次 `Credentials`；**agent 侧**上传遇 `AccessDenied` 时作废凭据、刷新并重试一次，二次仍 403 则落终态告警；② **模板解析失败改为任务失败而非猜键**（修 IC-BUG-21）；③ **规则同步补全集语义**（修 IC-BUG-30，M-2 扫描新增）：`SyncRulesOnConnect` 连 inactive 一起推（agent 的 `applyRule` 对 `Enabled == false` 已会 `stopRule`，复用既有分支），**并另下发一条「本次同步的 `rule_id` 全集」让 agent 停掉集合外的规则**——删除的行已不在 `ListCollectionRulesByAgent` 里，只推 inactive 修不掉删除那半；④ **`Connect` 里把发送 goroutine 提到 `SyncRulesOnConnect` / `pushCredentials` 之前启动**（IC-BUG-31 的结构半边：当前两处都在无消费者时入队，≥32 条 active 规则即静默丢弃），并让 `DispatchRule` 不再吞掉 `Send` 失败 | agent 与 CP 断连期间删除/停用规则，恢复连接后**不重启 agent** 也不再产生上传；运行中新建指向新 bucket 的规则，首个文件即成功（不出现 403）；配 40 条 active 规则重连后 40 条全部生效且凭据到达 | ✅ PR（IC-2b，2026-09-11）。**live 三条全过**（`TestIC2BLive`，三次独立运行）：act1 删除+停用（两种路径）不重启 agent 不再上传；act2 新 bucket 首文件成功、日志零 403；act3 40 条规则全部生效 + 凭据到达。**③ 落地为快照形态（D-033 补记）**：初版「逐条 push + 全集 ID」被行为级验证证伪——40 条 + 凭据经 32 缓冲 5/5 确定性丢 8 条 + 凭据（burst-vs-drain 结构性），与「快照失败断连」组合对 >32 规则 agent 是无限重连循环；改为一条 `RulesSyncCommand{rules}` 快照（`rules_sync = 17`，纯追加）。**深度 review（2026-09-11，PR #103）修四条**：F1 快照/增量竞态（per-agent 互斥锁，快照构建中途新建的规则不再被旧快照停掉）；F2 STS 凭据代际保护（迟到旧响应整体丢弃；二次 403 需代际稳定才落终态，被覆盖则用最新凭据再试一次）；F3 快照超限从响式失败改为降级保活 + `MaxRulesPerAgent`（响式失败=确定性无限重连循环，实测证伪）；F4 补行为级用例 `TestServer_Connect_SendConsumerRunsBeforeSync` 钉住「消费者先于同步启动」（快照形态下该变异已无消息丢失差异，原 40 条快照用例杀不死它——变异验证红绿各 2/2）。**已知残留（未修，待协调者立卡）**：经 API 批量改动大量规则时增量通道的 burst 会重现同样的缓冲丢弃；IC-BUG-21 的失败不落 `upload_logs`（UI 无行，可见性 = 去重 Warn），如需 UI 行需另刀；REST 删规则撞 `file_entries` FK（既有产品行为，本刀 live 测试绕过） |
| **IC-3** | agent + deploy | **续传落盘 + 分片清理**（IC-BUG-5 + IC-BUG-39）。① `Queue.SaveMultipartProgress(ctx,id,uploadID,partsJSON)`，initiate 与每片完成即落盘；② 终态清理：executor `AbandonFunc` hook——放弃（重试耗尽）/ 终态失败 / 任务被逐出时 `AbortMultipartUpload`；**`completed` 不 Abort**（CompleteMultipartUpload 已消费 uploadID）；续传正确性仍以 `verifyRemoteParts` 对账为权威，验证失败不 abort；③ 数据桶 `AbortIncompleteMultipartUpload` ILM 规则——⚠️ 当前 MinIO（≤2025-10-15 全代系，`internal/bucket/lifecycle/rule.go` 的 FIXME）不实现该 action：abort-only 400、共存被静默剥离；脚本照写 + 读回生效值 + 不生效响亮告警；④ IC-BUG-39 同刀关闭（Go 测试比对脚本与 `policy.go`，反向变异自验红）。**依赖 IC-1 已签发桶级 `s3:ListBucketMultipartUploads`** | live（`IC3_LIVE=1`，两次 PASS）：①320MB 中途 SIGKILL → 重启续传，日志 `skipped_parts=1`、upload_id 与 kill 前一致、非 part 1 重传、`file_entries` 一行；②retry_max=0 中途删源文件 → 放弃 → MinIO 无 incomplete 分片无对象、日志见 abort 行。**review 收尾（PR #105 round-2，4 P1 + 1 P2，均先红后绿+变异）**：对账仅对可确认 NoSuchUpload 才重新 initiate（瞬时失败原样返回等重试）；dedup 转 completed 前对非空 upload_id 做可重试的持久 abort；首次 SaveMultipartProgress 失败必 abort 刚建的上传（每片失败靠已持久化 ID + 远端对账，策略不对称已写明）；failed 退避持久化 `next_retry_at` 并在启动恢复（放弃/终态打 never-retry 哨兵不复活）；淘汰改为事务内先记 durable abort 记录再删行 + 后台 abort worker 带超时重试，误导性 ILM 日志已改——② 之外无任何兜底是该刀的既定事实 | ✅ |
| **IC-4** | CP + deploy | **webhook 可靠性止血**（IC-BUG-6 + IC-BUG-7 + IC-BUG-9；**IC-BUG-19 已于 2026-09-10 拆为独立的 IC-2c，排在 IC-2a 之前**）。① **失败语义 + 毒丸**。**⚠️ 不得按 4xx/5xx 分流**——dev 实测 MinIO 对 400 与 500 **一视同仁**（都走 `sendSync.func1()` 失败分支，日志 `returned '400 Bad Request'`）。原方案「解析失败返回 400，坏载荷不该无限重投」**不成立**：配上本刀 ③ 的持久 `queue_dir` 后，400 会被无限重投并**队头阻塞整条流**（实测 `queue_dir` 是队头阻塞单队列，失败事件重试期间后续 delete/create 全部排队、约 3s 一轮）。**统一走一条路径**：失败 → 计数 → 未超限返 5xx（MinIO 重投）→ 超限落死信 + 返 200 放行队列。三个必须定义清楚的实现点：**(a) 事件身份**——实测 webhook 请求头只有 `Host / User-Agent / Content-Length / Authorization / Content-Type`，**没有事件 ID、没有重试计数**，须用 `bucket + key + sequencer` 构造去重键；**(b) 计数器必须持久化**（Redis 或表）——放进程内存则毒丸打崩 CP 或运维重启后计数归零，永远到不了上限、毒丸永不失效（**已按此落地**：**PostgreSQL `webhook_fail_counters` 是权威单调计数**（迁移 000009，每次失败原子 upsert +1，重启存活），**Redis 仅作 best-effort 缓存**（写失败无害，读 miss 回落 PG）；进程内 map 与 `max(redis, pg)` 合并均已废除——三轮复审证明 max 是错的：两侧计数覆盖**不重叠时段**，取最大会吞失败次数，且跨两个权威的合并非原子）；**(c) 死信要落表并写明谁 redrive**，只落日志等于无声丢数据。**(c-2) 顺带把 IC-2c 的解码失败兜底改判到死信**（PR #96 评审建议，写在这里免得靠遗忘变成永久现状）：IC-2c 现在对解码失败的键「索引原值 + Warn」，理由是丢 create 比索引怪键更糟。该兜底**从 MinIO 实际上不可达**（`QueryEscape` 的输出必然是合法转义，解码失败即意味着载荷不是 MinIO 发的）**且是对称的**（create 与后续 delete 退回同一个原值串，软删仍能精确命中，不制造删不掉的幽灵）——所以当前形态无害。但本刀有了死信表之后，解码失败应当进死信而不是进索引，属严格改进。**⚠️ (d) 落死信的 `ObjectRemoved` 必须把对应分片强制置 `active`**——「靠 IC-13 兜底」对删除类事件**不成立**：丢失的 create 是「MinIO 有 / PG 无」，L2 兜底列举能找回；而丢失的 delete 是「PG 有 / MinIO 无」，属 L3 方向，但**删除不推进 `object_keys.last_modified`**（§3.5 坑 1 只准用它判「分片被写过」），已封存的归档分片将**永不解封**，那些行成为永久幽灵（UI 可见、预签名 404、三级对账一级都检不出）。死信表里有 `bucket+key`，强制置 active 的成本近乎零。**⚠️ 与本刀 ③ 的硬序**：③ 把 `queue_dir` 改成持久卷之后，失败事件才会真正无限重投——**① 必须与 ③ 同时或先于 ③ 生效**，否则中间态就是无限重投 + 队头阻塞。**注意与本条既有验收的张力**：「断开 PG → 5xx → 恢复后补齐」在上限过小时会失败，上限须大于预期的故障时长 ÷ 3s；② `MakeBucket` 后调 `SetBucketNotification`，并在启动时对 `buckets` 表逐个 ensure（幂等补注册）。**ensure 必须是幂等替换而非追加**（否则配置越攒越乱）。**⚠️ 2026-09-10 更正**：初版称「dev 上两条重叠订阅导致删除事件双投」——**实测证伪**，订阅确实是两条（同一 ARN），但 6 次 `mc rm` 精确产生 6 个 `ObjectRemoved` 事件，MinIO 按 ARN 归并、**没有双投**。要求本身保留，但**不要照着「双投」去设计幂等去重，那个问题不存在**。**ARN 须做成可配置**——IC-11 换 NATS 后 ARN 会变（`arn:minio:sqs::primary:webhook` → NATS target 的 ARN），写死会让 IC-11 把这条打回原形；③ `queue_dir` 迁至持久卷。**⚠️ 改脚本不等于生效**——dev 实测 `notify_webhook:primary` 的 `queue_dir` 实际为**空**，与 `init-minio.sh:125` 写的 `/tmp/minio-webhook-queue` 不符（`queue_dir` 为空时 MinIO 走 `sendSync`，投递失败即丢，日志可见 `Error: not connected to target server/service`）。验收须查**生效值**（`mc admin config get`）而非脚本文本 | 断开 PG 触发 ObjectCreated → 端点 5xx → 恢复 PG 后 MinIO 重投、`file_entries` 补齐；通过 API 新建 bucket 后直接 `mc cp` 一个对象，索引出现该行（② 未做，IC-4b） | ◐ **IC-4a 已收官（2026-09，随本 PR；二轮返工后：PG 兜底计数、oversized/malformed 分离、down 迁移实跑验证）**：① + (c-2) + (d) 标记 + ③ 全部落地并有 live 证据（断 PG → 5xx → 恢复重投；毒丸落死信表 + 200 放行；ObjectRemoved 死信 `active=true`；`queue_dir` 生效值 `/data/minio-webhook-queue` + 容器重启队列存活）。**变异矩阵 16 项护栏 BUILD_OK 验证各自被杀**。**(d) 的「强制置分片 active」目前只是死信行上的标记（IC-12 落地前无 shard_state 表），真正解封动作须随 IC-12 的 redrive 落地——已在卡片与 §3.6 写明。**②（bucket 通知 ensure / ARN 可配置 / IC-BUG-7）刻意未做，归 IC-4b，另行开刀** |
...
> **IC-5** ✅（PR #100）→（IC-2b / IC-3 / IC-4 / IC-SEC-2 可并行）——**这是当时的快照**；
> IC-2b（PR #103）与 IC-3（PR #105）此后均已收官，现候选见上文「下一刀怎么选」。
> **IC-2c 必须早于 IC-2a**（不是「宜早」，是硬序：见 IC-2c 一栏）——**该硬序已满足，IC-2a 已于 2026-09-11 收官**。
> ~~**下一刀在 IC-4 / IC-15 之间选**~~ ⚠️ **已作废，见顶部 banner**（彼此可并行；IC-3、D-034 已收官，IC-SEC-2 随 PR #109、IC-BUG-44/43 随 PR #108 收官）。IC-2a 的 live 验收过程中新立了
...
> `agent_id` 再发 cancel**——**开工前** handler 把 cancel 发给 URL 里的 agent，是 IC-BUG-29 第 6 条路径的另一半；✅ 已随 PR #109 修复）
...
### 地基 — 表结构（~~有时间窗口，宜早不宜迟~~ ⚠️ 排期已作废，见顶部 banner）（IC-6…IC-7）
...
| **IC-12** | CP worker | **L1 grant 结算对账**。到期未结算 / 数量不符的 grant → **不列举**，改为按该 grant 的时间窗口查 `object_keys.last_modified` 得出受影响分片 → 置 `active` 交 L2 预算核实 → **同时告警**（富字段 tags/run_id/sha256 已永久丢失，L2 只能补回存在性，无法恢复，必须让人知道）。**⚠️ (d) 依赖（PR #110 复审 B4）**：IC-4a 的死信表 `webhook_dead_letters` 已有 `active=true` 列标记 ObjectRemoved 死信，但**无消费方**——本刀必须实现死信 redrive 的读取路径（按 `active` 过滤、`created_at` 排序——已按此建索引 `idx_webhook_dead_letters_active_created`），对 redrive 的 ObjectRemoved 强制解封对应分片（置 `shard_state.active`），重放成功后删除该行；**该验收未完成前 IC-BUG-6 的 (d) 不得视为闭环**| 人为丢弃一次注册后，grant 到期时被检出、相关分片转 `active`、告警产生；**L1 全程不发一次 `ListObjects`**（不只是稳态） | ⬜ |
...
| ID | 模块 | 内容 | 验收要点 | 状态 |
|----|------|------|----------|------|
| **IC-15** | agent + CP + 产品 | **`append_mode=tail` 正确实现：滚动分块 + 服务端合并**（关 **IC-BUG-46** 的正确实现半边 + **IC-BUG-48**）。**IC-3 已先 fail-closed 挡掉 tail**（422 + 上传路径显式失败），本刀做对它。① **滚动块 = 一个 multipart part**：每次采集把新增字节作为下一个 part 上传，`upload_id` 活过整个采集周期（IC-3 ① 已把它持久化）；② **合并 = `CompleteMultipartUpload`**——服务端完成，**零重传零额外存储**；③ **重开已完成对象 = `ComposeObject` / UploadPartCopy**（**已在本项目 MinIO 实测通过**：8MiB + 6MiB → `size=14680064`、ETag 带 `-2`，服务端拼接零重传），用于「判定完整之后又来了新追加」的情形；④ **完整性判定**：主判据用**真 `IN_CLOSE_WRITE`**（今天的 `close_wait` 只是 500ms 空闲去抖，代码注释自己写着 "approximates"）+ **轮转检测**（写入方改名/新建），**静默超时作可配兜底**，**显式信号（哨兵文件 / API）作逃生舱**；⑤ **内容身份改为 ETag 验证**（IC-BUG-48）：已记录分片 ETag 与本地对应区间重算 MD5 比对，`mtime+size` 降级为预筛；⑥ **采集中的索引状态**：`Complete` 之前对象在桶里不存在，需定义一个独立状态（建议 `collecting`），`Complete` 后转正常并补齐 sha256/size——否则运维会看到「规则在跑但什么都没有」 | tail 规则采集一个持续追加的 >200MB 文件：每次只上传增量（带宽可验证）；判定完整后对象内容 = 完整文件且 sha256 正确；**判定完整之后再追加，对象能被正确扩展且不重传已有内容**；原地截断（`size < offset`）被识别为需 abort 重开而非「没有新字节」；同秒同尺寸改写能被重采（ETag 验证生效） | ⬜ |

> **为什么 tail 需要单独一刀而不是在 IC-3 里补**：tail 的「分片」是**时间上的增量段**，
> multipart 的「分片」是**空间上的固定切片**，两套概念此前被塞进同一条代码路径
> （所以 `multipartUpload` 会忽略 `offset` 整文件重传）。修正它需要定义完整性判定、
> 内容身份判据、以及采集中的索引状态——**这是一次功能设计，不是补丁**。
>
> **两个硬约束决定块大小**：part 非末片 **≥5MiB**（实测 `EntityTooSmall`）、
> **≤10000 个 part**（S3 规格）。故块大小应为规则级可配、下限 5MiB；
> 沿用现有 64MiB part size 可得 **640GB** 单文件上限（5MiB 则只有 50GB）。
>
> **③ 让完整性判定从「正确性要求」降级为「优化」**：判早了不丢数据，只多一次服务端拷贝；
> 判晚了只是对象晚出现。这大幅降低了 ④ 的难度——
> 原本「500ms 启发式差几个数量级」是阻塞性难题，现在是调参问题。
> ⚠️ 但 UploadPartCopy 的源在非末片位置必须 ≥5MiB；已完成对象小于 5MiB 时需退回下载+重传
> （该尺寸下无所谓）。另：每次重开都会重写对象（新 ETag / `LastModified`），
> 需确认与 IC-2a 的 upsert 语义相容。
