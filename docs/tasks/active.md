...
## ⏭️ 下一步（2026-09-24 拍板）：A 基线审计 — **在独立新会话执行**

IC-4a 随 PR #110 收官后，产品决定**暂停推进新功能，先盘点现状**。

**目标 A**（产品定义的「可运行版本」）：一台 agent + CP + Web UI，配一条采集规则，
**文件能采上去、索引里能查到、能下载**。其余（多 agent / 对账 / 标签 / SDK / 监控 / UI 重做）不算 A。

**工作章程**：[`audit-a-baseline.md`](audit-a-baseline.md) —— 该文件是**一把尺子，不是一份答案**，
刻意不含任何现状判断；执行者不得把其中任何一行当作「该功能存在」的背书。

⚠️ **必须在干净的新会话执行**：审计要回答「什么是现状」，带着既有结论进场会把结论当基线，
只审自己没断言过的部分——那正是本项目反复出问题的机制（章程 §2 有实证）。

⚠️ **现有的「挡 / 可推」判据不能沿用**：它 2026-09-11 重判过一次，判的是「挡着能不能扛住故障」，
**不是「挡着能不能用」**。本轮按 A 的标尺重新二分为 **挡 A / 不挡 A**。
...
- **下一步：A 基线审计章程**：[`audit-a-baseline.md`](audit-a-baseline.md)
