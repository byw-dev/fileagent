...
- **缺陷清单**：[`bugs/open.md`](bugs/open.md) IC-BUG-1…IC-BUG-32（8 个 P0 / 14 个 P1 / 10 个 P2；30…32 来自 2026-09-10 的 M-2 类扫描）
...
> **IC-2a 是不可再拆的原子刀**——IC-BUG-8（排序键）、IC-BUG-19（webhook 键 URL 解码）、
> IC-BUG-29（rule_id 归属）、IC-BUG-31 的 ack 半边都必须与「上报」同刀，否则每一条都会**引入新缺陷**
> 而非只是留着旧缺陷。原 IC-2 的 ⑪ 项已于 2026-09-10 拆为 IC-2a / IC-2b / IC-SEC-2，详见 track 文件。
