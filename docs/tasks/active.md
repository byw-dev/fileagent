...
> ~~**地基阶段越晚做越贵**——按 §6.7 换算约 2600 万对象/年~~ ⚠️ **该前提不成立**（2026-09-24 重判）：
> 系统尚未发布、没有任何生产数据在累积，时钟还没开始走。见「下一步」一节的「明确不做」。
...
`.github/workflows/ci-smoke.yml` —— 十二环 + webhook 探针，约 1 分钟，PR 必过。

> ⚠️ 两条会影响后续判断的事实：
> ① **「CI 绿」的含义被高估**：11 个 `//go:build integration` 文件从未在 CI 跑过，
> 另 4 个依赖 PG 的测试以 `ok`(SKIP) 绿着（`backlog.md:47` 已有账）。
...
| **2** | **让 CI 的绿灯可信** | 一天 | `ci-cp.yml` 加 `services:` + 跑 `-tags=integration`；webui 补 PR 门（22 个 vitest 文件至今无人运行）。照抄 `ci-agent.yml` 里 inotify overflow 那段的 **grep 防静默跳过**写法。已有账：`backlog.md:39-55` |
...
- **IC-6/7 地基提前**——卡片里「越晚越贵、2600 万对象/年」的前提**不成立**：尚未发布，没有数据在累积
