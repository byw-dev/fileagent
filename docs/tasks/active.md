...
- **缺陷清单**：[`bugs/open.md`](bugs/open.md) IC-BUG-1…IC-BUG-55（30…32 来自 2026-09-10 的 M-2 类扫描，33/34 来自同日评审，35 来自 IC-2c 的 live 验收，36 是 IC-2a 开工时挡路的那条，37/38 来自 IC-2a 的 live 验收，**39…41 来自 PR #97 的 code review**）。**55 条中已关闭 37 条**：随 IC-2a 关闭 **2/8/28/29/33**（另 13 关掉一半；**31 的两半此后已齐全**——结构半边随 IC-2b/PR #103），随 PR #97 关闭 **35/36**，随 **IC-5（PR #100）关闭 10/11/34/37 并补齐 12 的超时半边**（12 不再是拆半），随 **IC-2b（PR #103）关闭 20/21/30**，随 **IC-3（PR #105）关闭 5/39/47**（IC-3 的 ILM 兜底受上游 MinIO 限制，见卡片），随 **D-034（PR #106）关闭 50**，随 **PR #108 关闭 43/44**，**随 IC-SEC-2（PR #109）关闭 26/27**，随 **IC-4a 关闭 6/9**（webhook 失败语义+毒丸+死信表、`queue_dir` 持久卷；IC-BUG-7 归 IC-4b；**IC-BUG-6 的 (d) 只落 `active=true` 标记位，强制解封归 IC-12**）。**另已撤销 1 条（49）；未关闭 17 条：2 条挡（7/40）、12 条可推、3 条拆半（13、46 与 32）**（32 的拆半判于 PR #109：空闲路径随 IC-SEC-2 关闭、积压下送达保证未排期）（6/7 是「挡（收窄）」，仍算挡；42…45 是 PR #100 两轮 review 新立、**立卡时**均判可推（⚠️ **43/44 已随 PR #108 关闭**，「均判可推」现在只对 42/45 成立）；**46…49 是 IC-3 六轮 review 与 tail 设计讨论新立**——46 为拆半：出血半边已随 IC-3 挡掉、正确实现归 IC-15，**tail 当前不可用**；47 已随 IC-3 关闭；48 归 IC-15；**49 已撤销**——前提被实测证伪，其中成立的产品问题拆入 [`backlog.md`](backlog.md)「同一文件重复采集的覆盖语义」；**50 已随 D-034 关闭**；**51 新立**（🟠 P1，立卡 P2 经 PR #107 review 上调）——`sha256`/`size_bytes`/上传字节来自三次独立读，是 IC-11…13 对账的硬前置，宜与 48 同刀；**55 新立**（🟠 P1，不挡 A 故可推）——**失败**上报把 `upload_logs.bytes_transferred` 记成整文件大小，Web UI 失败行因此显示「已传输 <整个文件>」，反转了 D-027「展示部分进度」的意图；⚠️ 立卡时的「成功路径也不符」断言**已被 PR #121 复审证伪**（成功时 `result.SizeBytes` 会覆盖入队值），**成功路径与 `file_entries.size_bytes` 无问题**，**待第 3 步账本重判时定级**；**52 新立**——`dry_run_limit` 的有效上限恒为 10（CP 只在响应端裁剪、从不下发，agent 硬编码 10），PR #107 review 顺带撞见；⚠️ 立卡时误归为「参数收了不用」，**三轮 review 读码证伪**）——**判据已于 2026-09-11 重判**（「挡」的含义从「挡着能不能跑通」变成「挡着能不能扛住故障」），详见 [`consistency-ingest.md`](consistency-ingest.md) 「分诊结论」一节，**不要沿用旧口径的可推/挡**
...
- **G-A2 MinIO 镜像已从 Docker Hub 下架**，三个 compose 在任何全新环境都起不来
  ...
  ⚠️ **2026-09-25 续**：`quay.io` 这条路**也关了**（匿名取 manifest 401），
  当时写的「quay.io 是最后一条公共通路」已过期。现改为自持私有镜像仓并按 digest 钉定
  —— 见 [`DECISIONS.md`](../../DECISIONS.md) **D-036**。同一个缺陷第二次咬人，
  两次都是**由 CI 实跑撞出**、不是静态检查捞到的。
...
**落地记录（2026-09-25，PR #118，D-035）**：实际改法 = watcher 新增谓词
`debounceEnabled()`（= 模式 ≠ tail），三个分流点（`Start` 选循环 / `pollScan` 跳过热文件 /
`recheckAfterDebounce` 静默判据）全部改用它；`close_wait` 降为 `overwrite` 的别名
（值域三值不变）；`tail` 明确排除（fail-closed 不变，语义归 IC-15）。防抖窗口仍为
代码常量 500ms，未加配置。**live 证据（数字对）**：smoke.sh 新增 16MB 分块写护栏
（`cp` 直写监听目录，断言 `upload_logs` 恰好 1 行且 sha256 一致）——
**master（未改 watcher）6 行（红，6×16MB≈96MB 流量，同一写放大机制）→ 本分支 1 行（绿）**，
分支上 `SMOKE PASSED` 十二环全绿。6 条新增单测 + 5 条变异测试全部确认守卫有效；
`agent` 覆盖率 75.4% → 75.5%。已知代价（D-035「权衡」）：`overwrite` 交付即记 `seen`，
失去「下游静默失败后溢出重扫兜底重试」，与 `close_wait` 既有权衡一致（IC-BUG-53）。
