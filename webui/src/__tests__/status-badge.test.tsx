/**
 * Tests for the site-wide StatusBadge (§1.2 「圆点 + 文字」, contracts.md V-1).
...
describe('StatusBadge', () => {
  ...
  it('renders a colored dot matching the status tone', () => {
    const { container } = render(<StatusBadge status="REVOKED" />)
    const dot = container.querySelector('span > span') as HTMLElement
    // #D64545 error
    expect(dot.style.backgroundColor).toBe('rgb(214, 69, 69)')
    expect(statusColors.error.color).toBe('#D64545')
  })
})
