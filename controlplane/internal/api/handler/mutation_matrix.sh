...
# Run from controlplane/: bash internal/api/handler/mutation_matrix.sh
set -u
cd "$(dirname "$0")/.."
...
" "$file" "$old" "$new" || { echo "$name: MUTATION_APPLY_FAILED"; mv "$file.bak" "$file" 2>/dev/null; FAIL=$((FAIL+1)); return; }
  ...
  echo "$name: BUILD_OK"
  if go test ./internal/api/handler/ -run "$testpattern" -count=1 >/dev/null 2>&1; then
...
mutate_and_test "M1_shouldDeadLetter_ge" internal/api/handler/webhook_policy.go \
...
mutate_and_test "M2_default_limit" internal/api/handler/webhook_policy.go \
...
mutate_and_test "M3_identity_no_seq" internal/api/handler/webhook_policy.go \
...
mutate_and_test "M4_redis_key_nondeterministic" internal/api/handler/webhook_policy.go \
...
mutate_and_test "M5_no_clear_on_success" internal/api/handler/events.go \
...
mutate_and_test "M6_5xx_back_to_200" internal/api/handler/events.go \
...
mutate_and_test "M7_no_count" internal/api/handler/events.go \
